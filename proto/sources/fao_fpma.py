...
"""
FAO FPMA source adapter for V3.1 canonical observations.
...
def load_fao_fpma_records(path: Path) -> list[FaoFpmaRecord]:
    """
    Load raw FAO FPMA rows from CSV snapshot.

    Traceability:
    - PSwR-049
    - PSwR-039
    """
    rows: list[FaoFpmaRecord] = []
        ...
        for row in reader:
            rows.append(
                FaoFpmaRecord(
                    ...
                    unit=row["unit"].strip(),
                    provenance=row["provenance"].strip() or "FAO_FPMA",
...
def load_fao_fpma_observations(
    path: Path,
    *,
    ...
    """
    Load FAO FPMA as canonical observations.
    ...
    - PSwR-040
    """
    rows = load_fao_fpma_records(path)
    by_country: dict[str, list[FaoFpmaRecord]] = defaultdict(list)
    for row in rows:
